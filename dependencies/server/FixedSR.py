...
# listens for new clients
    ...
    def handleClientObjectUpdateField(self, datagram, dgi, targeted = False):
        ...
        if targeted:
            ...
            if not target:
                self.notify.warning(
                    "Ignoring targeted update to %s for %s on object %s from client %s: target not known" % (
                    targetId, dcfield.getName(), doId, client.doIdBase))
                return

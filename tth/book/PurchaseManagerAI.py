...
from tth.minigame import TravelGameGlobals
from tth.minigame import MinigameGlobals
...
class PurchaseManagerAI(DistributedObjectAI.DistributedObjectAI):
    ...
    def __init__(self, air, playerArray, mpArray, previousMinigameId, trolleyZone, newbieIdList = [], votesArray = None, metagameRound = -1, desiredNextGame = None):
        ...
        for avId in self.getInvolvedPlayerIds():
            if avId > 3 and self.air.doId2do.has_key(avId):
                ...
                av = gamebase.localAvatar[0]#self.air.doId2do[avId]
                avIndex = self.findAvIndex(avId)
                money = int(gamebase.toonAvatarStream.moneyMgr.getMoney())
                if avIndex == None:
                ...
                messenger.send('incomeMoney',[self.minigamePoints[avIndex]])
                ...
                if self.metagameRound == TravelGameGlobals.FinalMetagameRoundIndex:
                    ...
                    messenger.send('incomeMoney',[extraBeans])
    ...
    def requestExit(self):
        ...
        if self.receivingButtons:
            if self.air.doId2do.has_key(avId):
                av = gamebase.toonAvatar[0]#self.air.doId2do[avId]
                if avIndex == None:
        ...
        if self.receivingInventory:
            if self.air.doId2do.has_key(avId):
                ...
                if avIndex == None:
                    ...
                    currentMoney = gamebase.toonAvatarStream.moneyMgr.getMoney()
    ...
    def shutDown(self):
        if self.isShutdown:
            ...
            return None
        self.isShutdown = 1
        from tth.minigame import MinigameCreatorAI

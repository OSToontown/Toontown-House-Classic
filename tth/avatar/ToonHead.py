...
class ToonHead(Actor.Actor):
    ...
    # FIXME: This doesn't work on the server, as the server does not mount the
    #        phase files in a VFS (and it shouldn't). Find a way to fix this
    #        block of code on the server side, then remove this exception
    #        handler.
    try:
        # Default Eyes
        EyesOpen = loader.loadTexture(
            'phase_3/maps/eyes.jpg', 'phase_3/maps/eyes_a.rgb'
        )
    ...
    def generateToonHead(self, copy, style, lods, forGui=0):
        ...
        headType = headStyle[1:]

        filePrefix = ToonGlobals.HeadDict.get(headStyle[0])
        if not filePrefix:
            filePrefix = ToonGlobals.HeadDict.get(headStyle)
        if not filePrefix:
            self.notify.error('Unknown head style: %s' % headStyle)
        ...
        if len(lods) == 1:
            self.loadModel(
                'phase_3' + filePrefix + lods[0], 'head', 'lodRoot', copy
            )
            ...
            for lod in lods:
                self.loadModel(
                    'phase_3' + filePrefix + lod, 'head', lod, copy
                )
    ...
    def loadPumpkin(self, headStyle, lod, copy):
        if hasattr(base, 'launcher') and (
            not base.launcher or base.launcher and
            base.launcher.getPhaseComplete(4)):
    ...
    def __fixEyes(self, style, forGui=0):
        ...
        if not self.__eyes.isEmpty():
            ...
            if lp.isEmpty() or rp.isEmpty():
                self.notify.debug('Unable to locate pupils.')
            else:
                leye = self.__eyes.attachNewNode('leye')
                reye = self.__eyes.attachNewNode('reye')
                lmat = Mat4(
                    0.802174, 0.59709, 0, 0, -0.586191, 0.787531, 0.190197, 0,
                    0.113565, -0.152571, 0.981746, 0, -0.233634, 0.418062,
                    0.0196875, 1
                )
                leye.setMat(lmat)
                rmat = Mat4(
                    0.786788, -0.617224, 0, 0, 0.602836, 0.768447, 0.214658, 0,
                    -0.132492, -0.16889, 0.976689, 0, 0.233634, 0.418062,
                    0.0196875, 1
                )
                reye.setMat(rmat)
    ...
    def setupEyelashes(self, style):
        if style.getGender() == 'm':
            ...
            model = loader.loadModel(
                'phase_3' + ToonGlobals.EyelashDict[animal])
    ...
    def __blinkCloseEyes(self, task):
        if self.eyelids.getCurrentState().getName() != 'open':
            taskMgr.doMethodLater(
                4.0, self.__blinkCloseEyes, self.__blinkName
            )
        else:
            self.eyelids.request('closed')
            taskMgr.doMethodLater(
    ...
    def startBlink(self):
        ...
        taskMgr.doMethodLater(
            self.randGen.random()*4.0 + 1, self.__blinkCloseEyes,
            self.__blinkName
        )
    ...
    def lerpLookAt(self, point, time=1.0, blink=0):
        ...
        self.lookAtTrack = Parallel(
            Sequence(
                LerpPosInterval(
                    self.__lpupil, lookToTgtTime, endLpupil,
                    blendType=blendType
                ),
                Wait(0.5),
                LerpPosInterval(
                    self.__lpupil, returnToEyeCenterTime, origin,
                    blendType=blendType
                )
            ),
            Sequence(
                LerpPosInterval(
                    self.__rpupil, lookToTgtTime, endRpupil,
                    blendType=blendType
                ),
                Wait(0.5),
                LerpPosInterval(
                    self.__rpupil, returnToEyeCenterTime, origin,
                    blendType=blendType
                )
            ),
    ...
    def enterEyelidsClosed(self):
        if self.__eyes.isEmpty() or (not self.__eyesClosed):
            return None
    ...
    def enterEyelidsSurprised(self):
        if self.__eyes.isEmpty() or (not ToonHead.EyesSurprised):
            return None
            ...
            for lodName in self.getLODNames():
                ...
                if animal != 'dog':
                    ...
                    if lodName == '1000' or lodName == '500':
                        filePrefix = ToonGlobals.DogMuzzleDict[style.head]
    ...
    def isIgnoreCheesyEffect(self):
        if hasattr(self, 'savedCheesyEffect'):
            if self.savedCheesyEffect in (10, 11, 12, 13, 14):
                return True
        return False
